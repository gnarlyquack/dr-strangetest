...
function test_directory_targets(Context $context) {
    ...
    $test = easytest\discover_directory($state, new BufferingLogger($logger), $path);
    easytest\assert_falsy($logger->get_log()->get_events(), 'Errors during directory discovery');
    ...
    list($root, $targets) = easytest\process_user_targets($args, $errors);
    easytest\assert_falsy($errors, 'Errors during target processing');

    list($error, $actual) = easytest\find_directory_targets($logger, $test, $targets);
    $expected = array(
        array(
            'name' => "{$root}test1.php",
            'subtargets' => array()
